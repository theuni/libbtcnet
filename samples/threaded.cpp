#include "helpers/threadedconnman.h"
#include "helpers/threadednode.h"
...
class CConnMan : public CThreadedConnManager
{
public:
    CConnMan() : CThreadedConnManager(500000){}
private:
    void ProcessMessage(CThreadedNode* node, const std::vector<unsigned char>& inmsg);
    void ProcessFirstMessage(CThreadedNode* node, const std::vector<unsigned char>& inmsg);
    void OnReadyForFirstSend(CThreadedNode* node);
    ...
    template <typename T>
    void WriteCommand(CThreadedNode* node, const std::string& command, const T& obj, uint32_t checksum);
    void WriteCommand(CThreadedNode* node, const std::string& command);
};
...
template <typename T>
void CConnMan::WriteCommand(CThreadedNode* node, const std::string& command, const T& obj, uint32_t checksum)
{
...
void CConnMan::WriteCommand(CThreadedNode* node, const std::string& command)
{
    CNetStream buf(SER_NETWORK, node->GetSendVersion());
    const CNetworkConfig& netConfig = node->GetConnection().GetNetConfig();
    buf.resize(netConfig.header_size, 0);
...
void CConnMan::OnReadyForFirstSend(CThreadedNode* node)
{
...
void CConnMan::ProcessFirstMessage(CThreadedNode* node, const std::vector<unsigned char>& msg)
{
...
void CConnMan::ProcessMessage(CThreadedNode* node, const std::vector<unsigned char>& msg)
{
    CNetStream buf(msg.begin(), msg.end(), SER_NETWORK, node->GetRecvVersion());
    CMessageHeader hdr;
    buf >> hdr;
 ...
 */
    ...
    thread_type event_thread(&CConnMan::Run, &connman);
    connman.Start(8, 0);
    event_thread.join();
#ifdef _WIN32
    WSACleanup();
